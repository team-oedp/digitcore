...
export function truncateWithContext(
  ...
  const plainText = typeof text === 'string' ? text : extractTextFromPortableText(text);
  ...
  const end = Math.min(plainText.length, matchIndex + termLength + contextAfter);

  // Adjust start if we have room at the end
  const adjustedStart = Math.max(0, Math.min(start, plainText.length - maxLength));
...
export function highlightMatches(text: string, searchTerm: string): string {
  ...
  return plainText.replace(regex, '<mark class="bg-yellow-200 rounded-sm">$1</mark>');
}
...
export function getMatchExplanation(
  ...
  const descriptionMatch = plainDescription.toLowerCase().includes(searchTerm.toLowerCase().trim());
