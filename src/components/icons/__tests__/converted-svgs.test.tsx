...
describe('Converted SVG Files', () => {
  ...
  describe.each(svgFiles)('SVG file: %s', (fileName) => {
    ...
    it('should contain CSS custom properties for fill opacity', () => {
      expect(svgContent).toMatch(/fill-opacity="var\(--icon-fill-opacity,\s*[^)]+\)"/);
    });

    it('should contain CSS custom properties for stroke color', () => {
      expect(svgContent).toMatch(/stroke="var\(--icon-stroke-color,\s*[^)]+\)"/);
    });

    it('should contain CSS custom properties for stroke opacity', () => {
      expect(svgContent).toMatch(/stroke-opacity="var\(--icon-stroke-opacity,\s*[^)]+\)"/);
    });
    ...
    it('should preserve original values as fallbacks', () => {
      ...
      const originalFillOpacityMatch = backupContent.match(/fill-opacity="([^"]+)"/);
      const originalStrokeOpacityMatch = backupContent.match(/stroke-opacity="([^"]+)"/);
      ...
      if (originalFillMatch) {
        ...
        if (originalFill !== 'none') {
          expect(svgContent).toContain(`var(--icon-fill-color, ${originalFill})`);
        } else {
      ...
      if (originalStrokeMatch) {
        ...
        if (originalStroke !== 'none') {
          expect(svgContent).toContain(`var(--icon-stroke-color, ${originalStroke})`);
        } else {
          // 'none' values should remain unchanged
      ...
      if (originalFillOpacityMatch) {
        ...
        expect(svgContent).toContain(`var(--icon-fill-opacity, ${originalFillOpacity})`);
      }

      if (originalStrokeOpacityMatch) {
        const originalStrokeOpacity = originalStrokeOpacityMatch[1];
        expect(svgContent).toContain(`var(--icon-stroke-opacity, ${originalStrokeOpacity})`);
      }
    });
  ...
  it('should have consistent conversion pattern across all files', () => {
    ...
    conversionPatterns.forEach(pattern => {
      ...
      expect(pattern.hasStrokeOpacityVar).toBe(firstPattern.hasStrokeOpacityVar);
    });
  });
});