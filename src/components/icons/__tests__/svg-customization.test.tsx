...
describe("SVG CSS Custom Properties", () => {
	...
	it("applies default CSS custom properties correctly", () => {
		...
		const fillPath = container.querySelector(
			'path[fill*="var(--icon-fill-color"]',
		);
		const strokePath = container.querySelector(
			'path[stroke*="var(--icon-stroke-color"]',
		);
		...
		expect(fillPath?.getAttribute("fill")).toBe(
			"var(--icon-fill-color, #85A374)",
		);
		expect(fillPath?.getAttribute("fill-opacity")).toBe(
			"var(--icon-fill-opacity, 0.5)",
		);
		expect(strokePath?.getAttribute("stroke")).toBe(
			"var(--icon-stroke-color, #85A374)",
		);
		expect(strokePath?.getAttribute("stroke-opacity")).toBe(
			"var(--icon-stroke-opacity, 0.5)",
		);
	});
	...
	it("applies custom fill color via CSS custom property", () => {
		...
		expect(container.style.getPropertyValue("--icon-fill-color")).toBe(
			"#ff6b9d",
		);
	});
	...
	it("applies custom stroke color via CSS custom property", () => {
		...
		expect(container.style.getPropertyValue("--icon-stroke-color")).toBe(
			"#8b5cf6",
		);
	});
	...
	it("applies custom opacity values via CSS custom properties", () => {
		...
		expect(container.style.getPropertyValue("--icon-stroke-opacity")).toBe(
			"0.7",
		);
	});
...
describe("SVG Conversion Validation", () => {
	...
	it("validates that SVG elements have the correct attribute structure", () => {
		...
		expect(fillPath.getAttribute("fill-opacity")).toMatch(
			/var\(--icon-fill-opacity,\s*0\.5\)/,
		);
		...
		expect(strokePath.getAttribute("stroke-opacity")).toMatch(
			/var\(--icon-stroke-opacity,\s*0\.5\)/,
		);
	});
});
