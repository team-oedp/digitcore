...
describe('SVG Integration Tests', () => {
  ...
  it('preserves fallback values from original SVG', async () => {
    ...
    const originalPath = path.join(process.cwd(), 'public/icons-backup/icon-01.svg');
    ...
    const originalFillOpacityMatch = originalContent.match(/fill-opacity="([^"]+)"/);
    const originalStrokeOpacityMatch = originalContent.match(/stroke-opacity="([^"]+)"/);
    
    if (originalFillMatch && originalFillMatch[1] !== 'none') {
      expect(convertedContent).toContain(`var(--icon-fill-color, ${originalFillMatch[1]})`);
    }
    
    if (originalStrokeMatch && originalStrokeMatch[1] !== 'none') {
      expect(convertedContent).toContain(`var(--icon-stroke-color, ${originalStrokeMatch[1]})`);
    }
    
    if (originalFillOpacityMatch) {
      expect(convertedContent).toContain(`var(--icon-fill-opacity, ${originalFillOpacityMatch[1]})`);
    }
    
    if (originalStrokeOpacityMatch) {
      expect(convertedContent).toContain(`var(--icon-stroke-opacity, ${originalStrokeOpacityMatch[1]})`);
    }
  });
  ...
  it('handles edge cases in SVG content', () => {
    ...
    testCases.forEach(({ fillColor, strokeColor, expected }) => {
      const { unmount } = render(
        <CustomizableIcon 
          src="/icons/icon-01.svg"
          fillColor={fillColor}
