...
import { searchPatternsWithParams, type SearchResult } from "~/app/actions/search";
...
export function SearchClientWrapper() {
	const location = createLogLocation("search-client-wrapper.tsx", "SearchClientWrapper");
	...
	useEffect(() => {
		const performSearch = async () => {
			...
			if (currentSearchString === lastSearchParamsRef.current) {
				logger.debug("client", "Skipping duplicate search", { searchId, currentSearchString }, location);
				return;
			}
			...
			const hasFilters = parsedParams.audiences.length > 0 || 
							 parsedParams.themes.length > 0 || 
							 parsedParams.tags.length > 0;
			
			// If no search criteria, don't search - just show empty state
			if (!hasSearchTerm && !hasFilters) {
				logger.debug("client", "No search criteria - showing empty state", { searchId }, location);
			...
			logger.debug("client", "Starting client-side search", { 
				searchId, 
			...
			try {
				...
				logger.debug("client", "Validated search params", validatedParams, location);
				...
				if (parsedParams.searchTerm) {
					...
					logger.debug("client", "Added search term", parsedParams.searchTerm, location);
				}
				if (parsedParams.audiences.length > 0) {
					urlSearchParams.set("audiences", parsedParams.audiences.join(","));
					logger.debug("client", "Added audiences", parsedParams.audiences, location);
				}
				...
				logger.debug("client", "Calling server action with URLSearchParams", 
					Object.fromEntries(urlSearchParams.entries()), location);
				...
				logger.debug("client", "Search loading state set to false", { searchId }, location);
			}
		};
	...
	useEffect(() => {
		return () => {
			if (abortControllerRef.current) {
				abortControllerRef.current.abort();
				logger.debug("client", "SearchClientWrapper unmounted - aborting search", { searchId }, location);
			}
		};
	}, []);
	...
	const currentAudiences = searchParams.get("audiences")?.split(",").filter(Boolean) || [];
	const currentThemes = searchParams.get("themes")?.split(",").filter(Boolean) || [];
